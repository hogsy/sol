...
static const WCHAR WRAPPED_EXE[] = L"kmquake2_x64.exe";
#else
static const WCHAR WRAPPED_EXE[] = L"kmquake2.exe";
#endif
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	rsize_t				lastBackSlashSize = sizeof(exePath)/sizeof(WCHAR);
	WINDOWPROCESSINFO	info;
	int					maxLenSafe = sizeof(exePath)/sizeof(WCHAR) - (int)wcslen(WRAPPED_EXE);
	...
	else
	{
		...
		if (lastBackSlash != NULL) {
			lastBackSlash[1] = 0;
			lastBackSlashSize = sizeof(exePath)/sizeof(WCHAR) - (lastBackSlash - &exePath[0]);
		}
		else {
			...
			lastBackSlash[0] = 0;
			lastBackSlashSize = sizeof(exePath)/sizeof(WCHAR);
		}
	}
...
//	wcscat (lastBackSlash, WRAPPED_EXE);
	wcscat_s (lastBackSlash, lastBackSlashSize, WRAPPED_EXE);
