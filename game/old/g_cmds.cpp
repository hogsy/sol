...
void SaveEntProps (edict_t *e, FILE *f)
{
	...
	fprintf(f, "movetype    = 0x%08x\n"
		...
		e->classname.c_str(), e->spawnflags, e->moreflags, e->timestamp, e->angle, e->target,
	...
	fprintf(f, "speed       = %g\n"
		...
		(e->groundentity ? e->groundentity->classname.c_str() : "None") );
	...
	fprintf(f, "enemy = %s\n",(e->enemy ? e->enemy->classname.c_str() : "NULL"));
