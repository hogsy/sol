...
//	strncpy(modelname, gibname);
	...
	gib->key_message = static_cast<char*>(gi.TagMalloc (msgSize, TAG_LEVEL));
...
//	strncpy(modelname, gibname);
	...
	// Save gibname and type for level transition gibs
	msgSize = strlen(modelname)+1;
	self->key_message = static_cast<char*>(gi.TagMalloc (msgSize, TAG_LEVEL));
//	strncpy(self->key_message, modelname);
	...
	self->die = gib_die;
	self->dmgteam = nullptr; // Prevent gibs from becoming angry if their buddies are hurt
	self->postthink = nullptr;	// Knightmare- stop lava check
...
void barrel_explode (edict_t *self)
{
	...
	T_RadiusDamage (self, self->activator, self->dmg, nullptr, self->dmg+40, MOD_BARREL, -0.5);
...
void misc_viper_bomb_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	T_RadiusDamage (self, self, self->dmg, nullptr, self->dmg+40, MOD_BOMB, -0.5);
...
void misc_viper_bomb_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (self->pathtarget)
	{
		...
		{
			viper = G_Find( nullptr, FOFS(targetname), self->pathtarget);
			if (!viper)
				return;
...
void teleport_transition_ents (edict_t *transition, edict_t *teleporter, edict_t *destination)
{
	...
	for (i=game.maxclients+1; i<globals.num_edicts; i++)
	{
		...
		{
			if ( ent->owner == g_edicts[ j ].get() )
				ent->owner_id = g_edicts[ j ].get()->id;
		}
