...
void CMod_LoadSubmodels (lump_t *l)
{
	...
	dmodel_t *in = reinterpret_cast< dmodel_t * >( cmod_base + l->fileofs );
...
void CMod_LoadSurfaces (lump_t *l)
{
	...
	texinfo_t *in = reinterpret_cast< texinfo_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
	if (count < 1)
...
void CMod_LoadNodes (lump_t *l)
{
	...
	dnode_t *in = reinterpret_cast< dnode_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
void CMod_LoadBrushes (lump_t *l)
{
	...
	dbrush_t *in = reinterpret_cast< dbrush_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
void CMod_LoadLeafs (lump_t *l)
{
	...
	dleaf_t *in = reinterpret_cast< dleaf_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
void CMod_LoadPlanes (lump_t *l)
{
	...
	dplane_t *in = reinterpret_cast< dplane_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
{
	unsigned short *in = reinterpret_cast< unsigned short * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
void CMod_LoadBrushSides (lump_t *l)
{
	...
	dbrushside_t *in = reinterpret_cast< dbrushside_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
{
	darea_t *in = reinterpret_cast< darea_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
...
{
	dareaportal_t *in = reinterpret_cast< dareaportal_t * >( cmod_base + l->fileofs );
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	int count = l->filelen / sizeof( *in );
