...
char       *Hud_NameForDrawItem( hudDrawItem_t *drawItem )
{
	...
	if ( strlen( drawItem->name ) )
		snprintf( hud_itemname_buf, sizeof( hud_itemname_buf ), "%s '%s'", temp, drawItem->name );
	else
		snprintf( hud_itemname_buf, sizeof( hud_itemname_buf ), "%s", temp );
...
qboolean Hud_ParseDrawItem (hudItemGroup_t *itemGroup, char **script, char *buffer, int bufSize)
{
	...
	drawItem = itemGroup->drawItems[itemGroup->numDrawItems++] = static_cast< hudDrawItem_t * >( Z_Malloc( sizeof( hudDrawItem_t ) ) );
...
qboolean Hud_ParseItemGroup (hudDef_t *hudDef, char **script, char *buffer, int bufSize)
{
	...
	itemGroup = hudDef->itemGroups[hudDef->numItemGroups++] = static_cast< hudItemGroup_t * >( Z_Malloc( sizeof( hudItemGroup_t ) ) );
...
//	renameSet->maxRenameItems++;
	renameSet->renameItems = static_cast< hudRenameItem_t * >( Z_Malloc( ( renameSet->maxRenameItems + 1 ) * sizeof( hudRenameItem_t ) ) );
...
qboolean Hud_ParseHud (char *buffer, int bufSize)
{
	...
	snprintf (cl_hudDef.name, sizeof(cl_hudDef.name), cl_hudName);
...
void CL_LoadHud (qboolean startup)
{
	...
	// Load the file
	snprintf (fileName, sizeof(fileName), "scripts/huds/%s.hud", hudName);
	...
	for (i=0; i < foundSet->numRenameItems; i++)
	{
		...
		if ( wildcardfit(oldName, picName) )
		{
			...
			//	Com_Printf ("Hud_GetRenamedPic: Wildcard match: %s in %s to %s (%s -> %s)\n", oldName, picName, newName, oldPrefix, newPrefix);
				snprintf(renameBuf, sizeof(renameBuf), "%s%s", newPrefix, picName + strlen(oldPrefix));
				newPic = renameBuf;
				wc_renamed = true;

				// Add to rename wildcard cache
				addCacheItem = static_cast< hudRenameCacheItem_t * >( Z_Malloc( sizeof( hudRenameCacheItem_t ) ) );
...
void Hud_DrawItem (hudDrawItem_t *drawItem)
{
	...
	if ( drawItem->fromStat & (HUD_STATSHADER|HUD_WEAPONMODELSHADER) )
	{
		...
		if ( (drawItem->flags & DSF_OVERRIDEPATH) || (pic != orgPic) )
			snprintf (shader, sizeof(shader), "/gfx/huds/%s/%s.pcx", cl_hudDef.overridePath, pic);
		else
			snprintf (shader, sizeof(shader), "%s", pic);
	}
	else
		snprintf (shader, sizeof(shader), "/%s.pcx", drawItem->shader);
	...
	{	// no modding of tex coords for existing HUD pics (.pcx is FUBARed)
		...
		//	sh = th = 1.0f;
			...
			if ( (drawItem->flags & DSF_MASKSHADER) && strlen(drawItem->shaderMinus) > 0 ) {
				snprintf (shaderMinus, sizeof(shaderMinus), "/%s.pcx", drawItem->shaderMinus);
