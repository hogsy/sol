...
//	HANDLE  hMem;         /* handle to mem alloc'ed */
	...
	//	lstrcpy(szFullPath, ofs.szPathName);
		Com_strcpy (szFullPath, sizeof(szFullPath), ofs.szPathName);
	_lclose(hfile);
#endif
    ...
    char c, char *filename, char *methbuf, unsigned long crc, char fCrypt)
{
	...
	//	lstrcpy (szCompFactor, CompFactor100);
		Com_strcpy (szCompFactor, sizeof(szCompFactor), CompFactor100);
	else
	//	sprintf (szCompFactor, CompFactorStr, sgn, cfactor);
		Com_sprintf (szCompFactor, sizeof(szCompFactor), CompFactorStr, sgn, cfactor);
